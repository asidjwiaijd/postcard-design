...
export async function GET(req: NextRequest) {
  ...
  const rows = listSubmissions(status, 2000, 0);
  if (!rows.length) return NextResponse.json({ error: "这个状态下没有稿件" }, { status: 404 });

  const safe = (s: string) => s.replace(/[^\w一-龥-]/g, "_").slice(0, 30);
  const sources: ZipSource[] = [];

  const manifest = [
    ["序号", "编号", "QQ", "昵称", "正面朝向", "反面朝向", "状态", "已印", "提交时间", "正面文件", "反面文件"].join(","),
  ];
  ...
  rows.forEach((s, i) => {
    ...
    const base = `${n}_${safe(s.nickname || s.qq)}_${s.qq}`;
    ...
    manifest.push(
      [
        ...
        t.f, t.b, s.status, s.printed ? "是" : "否",
...
共 ${rows.length} 份稿件，每份一个文件夹，含正面 / 反面各一张。
清单见 清单.csv（UTF-8 BOM，Excel 可直接打开）。
